...
const fixture = fileURLToPath(new URL('./fixtures/promptaflow-mcp-fixture.mjs', import.meta.url))

async function target(mode, handler) {
  const path = await realpath(await mkdtemp(join(tmpdir(), `promptaflow-gateway-${mode}-`)))
...
function mcp(protocol = 'promptaflow-harness/2') {
  return message => {
...
test('concurrent sessions reuse one Hub workspace connection', async t => {
  ...
  const gateway = new PromptaFlowGateway(process.execPath, [fixture], globalThis.fetch, undefined, runtime.hubUrl)
...
test('generateAndRunGoal drives authoring and execution over MCP without a UI', async t => {
  ...
  const runtime = await target('generate-and-run', message => {
    if (message.method === 'initialize') return { protocolVersion: '2025-06-18', capabilities: {} }
    const name = message.params.name
    if (name === 'get_capabilities') return { structuredContent: { integration_protocol: 'promptaflow-harness/2' } }
...
test('an empty or malformed Session id never becomes an actor header', async t => {
  ...
  t.after(() => runtime.server.close())
  const gateway = new PromptaFlowGateway(process.execPath, [fixture])
