...
Use this reference after successfully installing or reinstalling Orbit. Report
...
## Orbit MCP App card catalogue

Codex and WorkBuddy render these five Orbit cards. The command is the tool whose
response carries the card; there is no separate open-card command.

| Card | Function | Command or tool | Copyable Chinese prompt |
| --- | --- | --- | --- |
| Orbit workspace | Goals, workflows, history, Agents, live progress, and attention state in one workspace view | `open_orbit_dashboard` | `打开 Orbit` |
| Orbit workflows | Published workflow catalogue; selecting an item shows its graph and definition in the same card | `list_workflows`; `get_workflow_definition` opens the same card at one workflow | `显示所有 Orbit 工作流` / `打开工作流 workflow:<id>` |
| Orbit workflow generation | One workflow authoring job's queue, generation, validation, publication, and result state | `generate_workflow` | `生成一个“开发—评审—测试—提交代码”的 Orbit 工作流` |
| Orbit goal execution | One Run's steps, attention state, human interruption, and final result | `start_run` | `用工作流 workflow:<id> 执行目标：<目标原文>` |
| Orbit goals | Recent goal Runs and their current statuses | `open_orbit_goals` | `打开 Orbit 目标列表` |
...
| --- | --- | --- |
| Open the full workspace card | `open_orbit_dashboard` | `打开 Orbit` |
| Show published workflows | `list_workflows` | `显示 Orbit 工作流列表` |
...
| Listen for Generate clicks from the full UI | `wait_authoring_request(client="codex-app")`; answer with `submit_authoring_response` | `现在监听 Orbit 的工作流生成请求` |
...
| Show recent goals | `open_orbit_goals` | `打开 Orbit 目标列表` |
| Resume delegated work on the first turn | `list_delegations`; ask before continuing when non-empty | `检查是否有可恢复的 Orbit 委托` |

Opening Orbit is display-only. It does not register `codex-app` or start an
authoring listener unless the user explicitly asks.

## DeepSeek Harness

DeepSeek Harness renders **no Orbit MCP App cards**. It installs a Host Profile
Bundle and renders its own resident Orbit panel instead. Describe the following
...
| --- | --- | --- |
| Open or focus the resident Orbit panel | `/orbit` | `打开 Orbit 面板` |
| Open the workflow picker and insert a workflow reference chip | `/orbit-workflows` | `选择一个 Orbit 工作流` |
| List workflows for the Agent | `orbit_list_workflows` | `列出当前 Workspace 的 Orbit 工作流` |
| List Runs | `orbit_list_runs` | `列出我最近的 Orbit 运行` |
| Inspect one Run | `orbit_inspect_run` | `查看 Orbit 运行 run:<id>` |
| Start a Run | `orbit_start_run` | `用工作流 workflow:<id> 执行目标：<目标原文>` |
| Cancel a Run | `orbit_cancel_run` after re-reading current state | `取消 Orbit 运行 run:<id>` |
| Resume an interrupted Run | `orbit_resume_run` with current revision and declared output | `继续 Orbit 运行 run:<id>，审批结果为通过` |
...
follow and report them. Workflow graphs, Artifacts, and authoring open Orbit's
...
WorkBuddy renders all five MCP App cards in the catalogue. It has no Orbit
plugin or stdio proxy; configure a custom Streamable HTTP connector named
`Orbit` at `http://127.0.0.1:8848/mcp` with no authentication.

| Intent | Tool sequence | Copyable Chinese prompt |
| --- | --- | --- |
| Select the project workspace | `list_workspaces`, then `select_workspace` | `列出 Orbit Workspaces，并选择 <项目名或绝对路径>` |
| Open the workspace card | `open_orbit_dashboard` | `打开 Orbit` |
| Show workflows as a card | `list_workflows` | `显示 Orbit 工作流列表` |
| Select or filter workflows without mounting a card | `inspect_workflows` | `找出所有可以直接启动目标的 Orbit 工作流` |
