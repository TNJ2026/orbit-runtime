# Authoring With The Current App

Address this App by the client name [hosts.md](hosts.md) gives for this host.
It is the name PromptaFlow displays and the name an author picks in the **Written
by** field. Use the same one in every call below: it is one address, and a
request addressed to a name this App did not register waits for nobody.

## Request starts in chat

When the user asks this task to generate a new workflow:

1. Call `register_authoring_client` with `client` set to your client name.
2. Call `generate_workflow` with the user's prompt, `agent` set to your client
   name, the user's display language, and a fresh idempotency key.
   This call opens the dedicated workflow-generation MCP App. Do not call
   `open_orbit_dashboard` first: the generation card shows only this job's
   progress and result.
3. Poll `claim_authoring_request` with `client` set to your client name until
   its `request` contains the generation prompt for that job.
4. Tell the user which authoring stage is running, follow the returned prompt
   exactly, and produce one Workflow DSL JSON document. Do not wrap it in
   Markdown. When a step needs an Agent and the returned Handler catalog has
   any `agent.*` Handler, bind the step to an appropriate available Agent CLI.
   Prefer CLI-backed Agents regardless of which App or Agent generated the
   workflow. Use `app.delegate` with `target: "run_initiator"` only when the
   catalog has no available Agent CLI Handler.
   When the requested Agent work has independently meaningful stages, express
   those stages as separate action nodes with connected, persisted outputs.
   Completed Workflow nodes are the recovery boundary; one monolithic Agent
   prompt cannot resume from its internal halfway point. Do not split work that
   is genuinely atomic or add nodes whose only purpose is naming a checkpoint.
   When the user explicitly numbers or names a sequence and requires every step
   to run in order or not be skipped, preserve each substantive step as its own
   business node and connect them in that order. Do not collapse that requested
   graph into an internal checklist in one Agent prompt. A terminal does not
   count as one of the requested steps; only a pure formatting step or one with
   no independently meaningful work or result may be merged.
5. Call `submit_authoring_response` with the returned `request_id` and DSL.
6. Call `get_authoring_job`. If compilation requests another attempt, repeat
   the claim, generation, and submission steps. Stop when the job is done,
   failed, cancelled, or the user stops it.
7. On success, report the published workflow name and identifier. A workflow
   list already visible in PromptaFlow should be refreshed before claiming it is
   up to date.

Narrate useful milestones in the conversation, but never paste hidden
reasoning. The authoring job's console and compiler diagnostics are the source
of truth for progress and failure.

## Request starts in the PromptaFlow UI

When the user wants a workflow generated by this task after clicking Generate
in PromptaFlow:

1. Call `wait_authoring_request` with `client` set to your client name and
   `timeout_seconds` set to 300. This is the case the blocking call is for —
   the user is going to the UI and the task has nothing else to do.
2. Tell the user to select your client name in the **Written by** field and
   click Generate while the tool call is waiting.
3. When the tool returns a request, follow its prompt exactly and produce one
   Workflow DSL JSON document. Do not wrap the document in Markdown.
4. While generating, watch the App event stream for `request_cancelled` with
   the same `request_id`. If received, abort the model request when the App
   supports cancellation, discard partial output, and do not submit it.
5. Call `submit_authoring_response` with the returned `request_id` and the DSL
   object.
6. Call `get_authoring_job` with the returned `job_id`. If compilation requests
   another attempt, wait again with the same client name and submit the revised
   document. Stop when the job is done, failed, cancelled, or the user stops it.

The wait call is what makes your client name visible in PromptaFlow. A timeout means no
request arrived; it is not a generation failure and may be started again.
PromptaFlow rejects a response submitted after cancellation even when the App cannot
interrupt its underlying model request.
