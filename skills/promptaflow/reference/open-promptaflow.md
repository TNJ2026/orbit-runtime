...
4. Tell the user where PromptaFlow opened and summarize the current or most recent
   run from the step 2 response. The full workflow catalog belongs in `/ui`.
Opening PromptaFlow is display-only. Do not call `register_authoring_client`,
`wait_authoring_request`, or otherwise announce this App as an author merely
because the user opened PromptaFlow. Register or listen only when the user explicitly
asks this App to receive workflow-authoring requests from the PromptaFlow UI; then
follow [authoring-with-current-app.md](authoring-with-current-app.md).

Tell the user where PromptaFlow opened and what the workspace card shows. Mention an
