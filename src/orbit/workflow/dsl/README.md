...
- **上一代在这一段里的输出、路由和 join deadline 会被作废**，而不只是等着被覆盖。否则回边目标下游的 join 会被上一代的分支值直接喂饱，或把上一代已经触发的 deadline 当成这一代的，在那些节点重跑之前就先“齐了”——链式图上看不出来，有 join 就会。唯一保留的是回边自己的源节点：它刚产出的值正是这条边要映射的东西。
...
最小形状是一条回边加一个有界 Policy：

```json
{
  "policies": [
    {"id": "bounded_rework", "kind": "rework",
  ...
  "edges": [
    {"id": "rejected",
     ...
     "condition": "source.submission.decision == 'reject'",
     "back_edge": true, "policy": "bounded_rework",
     "mapping": {"schema_id": "example://brief/1.0",
                 "value": "$source.submission.value"}}
  ]
}
```

可运行的完整例子是 `tests/fixtures/workflow_dsl/v1/human-rework.json`，由
`tests/test_workflow_rework.py` 驱动。它比上面这个形状大：审批后面接了 join
和 decision 节点，因为上一条规则（作废上一代的输出）只有在回边下游存在 join
时才观察得到，这个 fixture 同时是那条规则的回归用例。读语义看上面的片段，读
一个真的能跑的图看那个文件。
