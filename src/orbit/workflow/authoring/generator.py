...
class WorkflowAuthoringService:
    ...
    def _rules(
        ...
        shape_rules = [
            ...
            "Check an agent.* action's outcome wherever a later step depends on it having actually done something — written a file, changed code, or produced an inline deliverable another node consumes. For an inline result, follow that action with a decision node built from `agent_outcome`: ask for the token in the action's own config.prompt, branch on it, and route the failure branch to a terminal instead of letting the run continue on a report of the failure. Never apply `agent_outcome` to an artifact_ref result: an Artifact reference carries metadata, not readable `.text`, so that condition can never observe the token. When the Artifact itself is the requested deliverable, route it directly to the success terminal; when a later step truly requires a success decision, use a separate short inline status output from a Handler that declares one. Nothing else can see the difference — the Agent's CLI exits successfully whether it did the work or explained that it could not. Do not put a check after every action; one on the step whose silent failure would otherwise be inherited by everything downstream is worth more than a decision after each.",
            ...
            "n_of_m counts the edges arriving at ONE input port, not the branches reaching the join. Give the branches a single shared input port and let all of them target it; a port per branch means every port has one edge, and a threshold above 1 can never be met — the run is refused at start with 'cannot satisfy threshold'.",
            "Use join mode deadline only when a branch may never arrive at all — a human step nobody answers is the case it exists for. Branches that always finish need all, any or n_of_m: a deadline join whose inputs all complete has nothing to wait for, reaches no terminal, and fails the run.",
    ...
    def _prompt(
        ...
        facts = {
            ...
            "agent_outcome": {
                ...
                "scope": (
                    "Inline Agent results only. Never read `.text` from an "
                    "artifact_ref; route a final Artifact directly to success "
                    "or use a separately declared inline status output."
                ),
                "asking_for_it": (
