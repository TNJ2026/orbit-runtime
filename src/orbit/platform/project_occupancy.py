...
class ProjectOccupancyRegistry:
    ...
    def resolve(self, run_id: str) -> tuple[str, ...]:
        ...
        with _FileLock(self.root / REGISTRY_LOCK_NAME):
            ...
            for item in errors:
                if item["record_id"].endswith(f".{suffix}.json"):
                    self._quarantine_record(item["record_id"])
                    cleared.append(run_id)
        return tuple(cleared)

    def _quarantine_record(self, record_id: str) -> None:
    ...
    def resolve_record(self, record_id: str, *, processes_stopped: bool = False) -> None:
        """Repair one corrupt record after confirming its Agent processes stopped."""
        if not processes_stopped:
            raise ProjectNeedsRecovery("confirm Agent processes are stopped before repair")
        with _FileLock(self.root / REGISTRY_LOCK_NAME):
            errors: list = []
            ...
            self._quarantine_record(record_id)
    ...
    def inspect(self, path: Path | str) -> tuple[tuple[Occupancy, ...], tuple[dict, ...]]:
        ...
        resolved = Path(path).expanduser().resolve()
        identity = ProjectIdentity(resolved)
