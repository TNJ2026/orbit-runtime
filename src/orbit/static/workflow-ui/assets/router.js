 ...
 *
 * Target route shapes: `#/home`, `#/history/{id}`, `#/runs/{id}`,
...
export function readRoute(hash = location.hash) {
  ...
  // `goals` remains readable for old bookmarks; every newly generated URL
  // uses the clearer `history` path below.
  if ((parts[0] === "history" || parts[0] === "goals") && parts[1]) {
    return { view: "goal", runId: decodeURIComponent(parts[1]) };
  }
  if (parts[0] === "history") return { view: "goals", runId: null };
...
export function routeHash(route) {
  ...
  if (route.view === "goal") return `#/history/${encodeURIComponent(route.runId)}`;
  if (route.view === "goals") return "#/history";
