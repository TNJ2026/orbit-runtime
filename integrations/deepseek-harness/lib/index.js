...
function isUnknownToolError(error, tool) {
...
function decodeToolResult(name, value) {
	...
	if (name === "list_agents") {
		for (const [index, agent] of array(item.agents, "agents").entries()) {
			...
			if (entry.attempt_count !== void 0) number(entry.attempt_count, `agents[${index}].attempt_count`);
			if (entry.failed_count !== void 0) number(entry.failed_count, `agents[${index}].failed_count`);
		}
		return item;
	}
...
*/
const ORBIT_RPC_TIMEOUT_MS = 6e4;
var OrbitGateway = class {
	command;
	commandPrefix;
	fetchImpl;
	discoveryRoot;
	...
	constructor(command = "orbit", commandPrefix = [], fetchImpl = globalThis.fetch, discoveryRoot = process.env.ORBIT_RUNTIME_ROOT || void 0, hubUrl = process.env.ORBIT_HUB_URL || "http://127.0.0.1:8848") {
	...
	*/
	...
	async registerWorkspace(workspaceRoot) {
		const output = await this.runOrbit([
			"hub",
			"register",
			workspaceRoot
		], workspaceRoot);
...
var OrbitRemoteService = class extends TypertRemoteService {
	static inject = [
		"sessions",
		"workspaceRegistry",
		"tools",
		...
		ctx.effect(() => systemPrompt.context({
			...
			text: (context) => {
				...
				if (!this.recoveryPromptedSessions.has(sessionId)) {
					...
					recovery = "[ORBIT SESSION RECOVERY]\nOn this Session's first user turn, call orbit_list_delegations once. If the returned list is empty, do not mention recovery. If it contains work, tell the user what is resumable and ask whether to continue or reconcile it. Never execute an unknown delegation again.";
				}
	...
	*/
	...
	@Remote("getAuthoringOutput") async getAuthoringOutput(sessionId, outputHref, after, signal) {
		...
		const mount = () => {
			...
			*/
			ctx.effect(() => webServer.register({
				kind: "exact",
				path: "/plugins/dsh-orbit/artifact",
	...
	*/
	...
	@Remote("getRuntime") async getRuntime(workspace, signal) {
		signal.throwIfAborted();
	...
	@Remote("getRuntimeUi") async getRuntimeUi(sessionId, signal) {
		signal.throwIfAborted();
		const scope = await this.sessionWorkspace(sessionId, true);
		const release = await this.gateway.acquire(scope);
		try {
	...
	*/
	@Remote("getPanelState") async getPanelState(sessionId, force, startIfMissing, signal) {
		signal.throwIfAborted();
		...
		try {
			...
			const attemptCounts = listed.agents.some((agent) => agent.attempt_count === void 0 || agent.failed_count === void 0) ? await this.gateway.handlerAttemptCounts(scope, sessionId) : void 0;
	...
	*/
	@Remote("getRunDetail") async getRunDetail(sessionId, runId, signal) {
		signal.throwIfAborted();
		const scope = await this.sessionWorkspace(sessionId, true);
		const release = await this.gateway.acquire(scope);
		try {
	...
	*/
	@Remote("getWorkflowDefinition") async getWorkflowDefinition(sessionId, workflowId, signal) {
		signal.throwIfAborted();
		const scope = await this.sessionWorkspace(sessionId, true);
		const release = await this.gateway.acquire(scope);
		try {
	...
	@Remote("getStepOutput") async getStepOutput(sessionId, runId, nodeId, after, signal) {
		signal.throwIfAborted();
		const scope = await this.sessionWorkspace(sessionId, true);
		const release = await this.gateway.acquire(scope);
		try {
	...
	*/
	@Remote("runCommand") async runCommand(sessionId, runId, command, expectedRevision, value, interruptId, signal) {
		signal.throwIfAborted();
		const scope = await this.sessionWorkspace(sessionId);
		const release = await this.gateway.acquire(scope);
		try {
	...
	@Remote("reconcileStep") async reconcileStep(sessionId, runId, delegationId, outcome, note, signal) {
		signal.throwIfAborted();
		const scope = await this.sessionWorkspace(sessionId);
		const release = await this.gateway.acquire(scope);
		try {
	...
	*/
	@Remote("stopRuntime") async stopRuntime(sessionId, signal) {
		signal.throwIfAborted();
		const scope = await this.sessionWorkspace(sessionId);
	...
	@Remote("getDiagnostics") async getDiagnostics(workspace, sessionId, signal) {
	...
	@Remote("listWorkflows") async listWorkflows(workspace, sessionId, signal) {
		return await this.readListField(workspace, sessionId, "list_workflows", "workflows", {}, signal);
	}
	@Remote("listRuns") async listRuns(workspace, sessionId, status, signal) {
	...
	@Remote("generateWorkflow") async generateWorkflow(workspace, sessionId, prompt, signal) {
		signal.throwIfAborted();
	...
	@Remote("generateWorkflowForSession") async generateWorkflowForSession(sessionId, prompt, signal) {
		signal.throwIfAborted();
		if (!prompt.trim() || prompt.length > 2e4) throw new Error("Workflow prompt must be 1-20000 characters");
		const scope = await this.sessionWorkspace(sessionId);
	...
	@Remote("modifyWorkflow") async modifyWorkflow(workspace, sessionId, workflowId, prompt, regenerate, signal) {
		signal.throwIfAborted();
	...
	@Remote("getAuthoringJob") async getAuthoringJob(workspace, sessionId, jobId, signal) {
		signal.throwIfAborted();
		const scope = await this.verified(workspace, sessionId);
		return await this.gateway.call(scope, sessionId, "get_authoring_job", { job_id: jobId });
	}
	@Remote("getRun") async getRun(workspace, sessionId, runId, signal) {
		signal.throwIfAborted();
		const scope = await this.verified(workspace, sessionId);
		const release = await this.gateway.acquire(scope);
		try {
	...
	@Remote("getSteps") async getSteps(workspace, sessionId, runId, signal) {
		return await this.readRunField(workspace, sessionId, runId, "get_run_steps", "steps", signal);
	}
	@Remote("getGraph") async getGraph(workspace, sessionId, runId, signal) {
		return await this.readRunField(workspace, sessionId, runId, "get_run_graph", "graph", signal);
	}
	@Remote("getEdges") async getEdges(workspace, sessionId, runId, signal) {
		return await this.readRunField(workspace, sessionId, runId, "get_run_edges", "edges", signal);
	}
	@Remote("readOutput") async readOutput(workspace, sessionId, runId, after, nodeId, signal) {
		signal.throwIfAborted();
		const scope = await this.verified(workspace, sessionId);
		const release = await this.gateway.acquire(scope);
		try {
	...
	@Remote("listArtifacts") async listArtifacts(workspace, sessionId, runId, signal) {
		return await this.readListField(workspace, sessionId, "list_artifacts", "artifacts", {
			limit: 100,
			...runId ? { run_id: runId } : {}
		}, signal);
	}
	@Remote("getArtifact") async getArtifact(workspace, sessionId, artifactId, signal) {
		signal.throwIfAborted();
		const scope = await this.verified(workspace, sessionId);
		const release = await this.gateway.acquire(scope);
		try {
	...
	@Remote("getArtifactContent") async getArtifactContent(workspace, sessionId, artifactId, signal) {
		signal.throwIfAborted();
		const scope = await this.verified(workspace, sessionId);
		const release = await this.gateway.acquire(scope);
		try {
	...
	* fetching.
	*/
	@Remote("readArtifactText") async readArtifactText(sessionId, artifactId, signal) {
		signal.throwIfAborted();
		const scope = await this.sessionWorkspace(sessionId, true);
	...
	*/
	@Remote("exportArtifact") async exportArtifact(sessionId, artifactId, signal) {
		signal.throwIfAborted();
		const scope = await this.sessionWorkspace(sessionId);
		const held = await this.gateway.call(scope, sessionId, "read_artifact_content", { artifact_id: artifactId });
	...
	@Remote("importArtifact") async importArtifact(workspace, sessionId, artifactId, signal) {
	...
	@Remote("reconcileDelegation") async reconcileDelegation(workspace, sessionId, runId, delegationId, outcome, note, signal) {
		signal.throwIfAborted();
		const scope = await this.verified(workspace, sessionId);
		const release = await this.gateway.acquire(scope);
		try {
	...
	@Remote("executeCommand") async executeCommand(request, signal) {
		signal.throwIfAborted();
