...
export function effectManifest(before, after) {
    ...
    for (const path of [...paths].sort()) {
        ...
        if (JSON.stringify(prior) === JSON.stringify(current))
            continue;
        if (current === undefined || current.code.includes('D'))
            deletedFiles.push(path);
        else if (prior === undefined && (current.code === '??' || current.code.includes('A')))
