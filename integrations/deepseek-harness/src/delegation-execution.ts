...
import { effectManifest, snapshotWorkspace, type WorkspaceSnapshot } from './effects.js'
...
export async function executeDelegation(
  ...
  ports: DelegationExecutionPorts,
): Promise<void> {
  ...
  if (refusal !== undefined) { await ports.settle(undefined, refusal); return }
  ...
  try {
    ...
    renewal = setInterval(() => {
      void ports.renew().then(item => {
        if (item.cancel_requested) controller.abort()
