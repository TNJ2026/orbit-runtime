...
export function projectRootFrom(argv: readonly string[], cwd: string): string {
...
export function orbitCommandFrom(
...
async function main(): Promise<void> {
  ...
  const gateway = new OrbitGateway(
    orbitCommandFrom(argv, process.env), [], undefined, undefined, 'full',
  )
  ...
  const handle = (line: string): void => {
    queue = queue.then(async () => {
      ...
      if (expectsReply(message)) process.stdout.write(frame(reply))
    })
  }

  process.stdin.on('data', chunk => { for (const line of reader.push(String(chunk))) handle(line) })
...
export function isEntryPoint(
  moduleUrl: string, invokedAs: string | undefined,
): boolean {
  if (invokedAs === undefined || invokedAs === '') return false
