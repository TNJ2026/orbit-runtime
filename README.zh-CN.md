...
3. 在只读流程图中确认步骤，输入目标并点击 **开始执行**。
4. Orbit 将模板绑定到当前唯一 Agent Handler，保存本次 Run 的图快照并直接交给 LangGraph。

单 Agent 模式不生成 DSL、草稿或工作流版本，也不需要发布。模板只影响新 Run；
已经启动的 Run 始终使用自己的图快照恢复。多 Agent UI 继续保留完整工作流建模能力。
