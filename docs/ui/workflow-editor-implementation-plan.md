...
投影轮询也不会在编辑期间销毁本地 source。应用层 Gate 覆盖 256 KiB 边界、30 个无效节点
的大量 diagnostics 和基于同一 SQLite 文件的服务重建恢复；浏览器 Gate 覆盖键盘、离线保存、
重连及 reload 恢复，既有 Gate 覆盖多标签 revision 冲突。视觉回归新增 Editor 深浅主题 ×
360×800、768×900、1280×800 六张响应式基线及八张关键状态基线；发布前以完整测试套件、console error
检查和重复 migration 检查收口。

补充收口（2026-07-21）：结构化 Node 编辑扩展为新增、删除和重命名，Node ID 变化会
同步更新 Entry、Terminal 与 Edge 引用；新增节点可显式设置 Entry，删除节点同时移除关联
Edge。Workflow Detail 现提供完整版本历史，可查看任意已发布版本、用历史版本启动可重放
Goal，或从所选版本派生 Draft。one-active 冲突提供“继续现有草稿”和“废弃后从所选版本
新建”两个明确动作；发布冲突提供复制本地草稿、查看最新版、废弃并基于最新版新建。
Source Editor 增加 JSON 格式化，Diagnostics 增加错误码与 JSON Path 复制。视觉矩阵补齐
...
- YAML Draft 保留原始 source 并可在 Source 模式保存/校验；结构化编辑与 Format 操作统一
  产生格式化 JSON，不承诺保留 YAML 注释或排版。
...
- Source 文本采用 800 ms 自动 Save、用户显式 Validate；结构化表单 Apply 后自动 Validate。
