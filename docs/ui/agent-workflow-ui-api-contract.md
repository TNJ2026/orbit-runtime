...
| U0.3 | 挂载 `/api/v1` 与 `/ui` | `orbit serve` 启动后真实 HTTP smoke test 通过 |
...
```text
orbit serve
├─ /ui          模块化 Runtime UI（src/orbit/static/workflow-ui/）
├─ /api/v1
├─ /mcp
└─ /health
```

旧 `/workflow-ui` 原型路由与旧引擎 `server.py` 已在 M6 删除。Runtime 逻辑位于 `src/orbit/workflow/`，由唯一组合根 `web/app.py` 挂载。U0.3 必须验证：
...
8. `/ui` 与 `/api/v1` 在真实 `orbit serve` 进程下启动、访问和关闭。
