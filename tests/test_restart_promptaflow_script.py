...
class RestartPromptaflowScriptTests(unittest.TestCase):
    ...
    def test_the_console_script_form_is_recognised_too(self) -> None:
        ...
        Both are this program. Matching only the module form would leave a Hub
        started by `start-promptaflow.sh` running, holding 8848, while the
        restart reported there was nothing to stop.
        """
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            environment = self.environment(
                ...
                recorded=6100,
                ps_answers={
                    6100: "/x/.venv/bin/paf hub serve",
                    6101: "/x/.venv/bin/paf serve --project-root /a",
                },
            )
