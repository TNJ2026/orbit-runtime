...
class HandshakeTests(ApiTestCase):
    ...
    def test_dashboard_resource_is_discoverable_and_readable(self) -> None:
        with AsgiHarness(self.app) as client:
            ...
            self.assertEqual(
                {
                    "ui://orbit/current-task-v42.html", "ui://orbit/workflows-v19.html",
...
class DiscoveryTests(ApiTestCase):
    def test_tools_are_discoverable_with_schemas(self) -> None:
        with AsgiHarness(self.app) as client:
            ...
            self.assertEqual(
                "ui://orbit/current-task-v42.html",
                dashboard["_meta"]["ui"]["resourceUri"],
            )
            self.assertEqual(
                {
                    "open_orbit_dashboard": "ui://orbit/current-task-v42.html",
            ...
            self.assertEqual("ui://orbit/workflows-v19.html", card_bindings["list_workflows"])
            self.assertEqual(
                "ui://orbit/workflows-v19.html",
                card_bindings["get_workflow_definition"],
            )
            self.assertIsNone(card_bindings["inspect_workflow_definition"])
            self.assertEqual("ui://orbit/workflow-authoring-v11.html", card_bindings["generate_workflow"])
            self.assertEqual("ui://orbit/goal-run-v17.html", card_bindings["start_run"])
            self.assertEqual("ui://orbit/goals-v11.html", card_bindings["open_orbit_goals"])
            for item in tools:
    ...
    def test_app_resources_request_borderless_host_chrome(self) -> None:
        with AsgiHarness(self.app) as client:
            ...
            detail = next(
                ...
                if item["uri"] == "ui://orbit/workflows-v19.html"
            )
            ...
            read = rpc(
                ...
                {"uri": "ui://orbit/workflows-v19.html"}, actor="reader",
    ...
    def test_workbuddy_prompt_modes_distinguish_editable_and_direct_actions(self) -> None:
        with AsgiHarness(self.app) as client:
            ...
            dashboard = rpc(
                client, "resources/read",
                {"uri": "ui://orbit/current-task-v42.html"}, actor="reader",
            ).json()["result"]["contents"][0]["text"]
