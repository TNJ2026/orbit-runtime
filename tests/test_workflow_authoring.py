...
class AuthoringServiceTests(unittest.TestCase):
    ...
    def test_a_valid_fenced_answer_compiles_on_the_first_attempt(self) -> None:
        ...
        generated = json.loads(outcome.source)
        self.assertEqual("generated", generated["metadata"]["id"])
        access = next(
            item for item in generated["policies"]
            if item["kind"] == "workspace_access"
        )
        self.assertEqual({}, access["config"])

    def test_the_prompt_requires_workspace_access_on_new_workflows(self) -> None:

        model = ScriptedModel([json.dumps(valid_document())])
        service(model).generate("请把审批流程画出来")
        self.assertIn("workspace_access", model.prompts[0])
        self.assertIn("entry action", model.prompts[0])
